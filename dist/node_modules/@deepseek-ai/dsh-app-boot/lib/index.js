...
async function mountRootInclude(ctx, absoluteConfigPath, patches = [], bareModuleBaseUrl) {
	...
	// Plugins may also add entries at runtime through `ctx.loader.create`, which
	// the Loader's own root group imports with the Loader's base URL. A host
	// that owns the complete plugin set must resolve those bare names the same
	// way, or they fall back to the configuration directory.
	if (bareModuleBaseUrl !== void 0) {
		const loader = ctx.loader;
		const importFromConfig = loader.import.bind(loader);
		loader.import = (name, getOuterStack) => {
			if (name.startsWith(".") || name.startsWith("cordis:") || isAbsolute(name) || loader.internal === void 0) return importFromConfig(name, getOuterStack);
			return loader.internal.import(name, bareModuleBaseUrl, {});
		};
	}
	const rootInclude = {
		id: "include",
