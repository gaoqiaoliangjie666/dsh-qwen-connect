...
var JsonlSessionPersistence = class extends SessionPersistence {
	...
	* Removes the exact log file and leaves the shared project directory in
	* place; a session that never materialized has nothing durable to remove.
		...
		for (const project of await this.listProjectDirs(signal)) {
			...
			for (const dir of await this.listSessionDirs(project, signal)) {
				...
				try {
					...
					signal?.throwIfAborted();
					// One unreadable log must not make every other session (and the workspace) unlistable.
